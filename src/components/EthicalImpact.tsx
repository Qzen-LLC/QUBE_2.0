...
import { Stage, QuestionType } from '@/generated/prisma';
...
export default function EthicalImpact({ value, onChange, questions, questionsLoading, questionAnswers, onAnswerChange }: Props) {
  ...
  if (questions === undefined || questionsLoading === undefined) {
    console.warn('[EthicalImpact] Received undefined props - showing loader.', {
  ...
  return (
    <div className="space-y-10">
      <div className="bg-gradient-to-r from-primary/10 via-primary/20 to-primary/10 border-l-4 border-primary p-4 mb-8 rounded-2xl flex items-center gap-3 shadow-md">
        <div className="font-semibold text-foreground text-lg mb-1">Ethical Impact Assessment</div>
        <div className="text-muted-foreground">Evaluate the ethical requirements and constraints for implementing this AI solution.</div>
      </div>
      ...
      {questionsLoading || !questions ? (
        ...
        questions.map((q) => {
          ...
          if (q.stage === Stage.ETHICAL_IMPACT) {
